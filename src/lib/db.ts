...
// local_store.dart) over the OFFICIAL sqlite3 WASM build with the OPFS
// AccessHandle VFS. Schema and semantics are identical to the Flutter app
// (schemaVersion 3 layout); falls back to an in-memory DB when OPFS is
// unavailable (e.g. non-secure context) so the app still works network-only.
...
async function _open(): Promise<LocalStore> {
  ...
  try {
    ...
    console.warn('[db] OPFS open failed, falling back to memory:', e)
    db = new sqlite3.oo1.DB(':memory:', 'c')
  }
